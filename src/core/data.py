...
def get_img_765bands(path):
    # img = rasterio.open(path).read((7,6,5))
    img = rasterio.open(path).read((7,6,5)).transpose((1, 2, 0))    
    img = np.float32(img)/MAX_LANDSAT_PIXEL_VALUE
    # img = np.float32(img)
...
def open_landsat_image_and_mask(paths):
    image_path = paths[0].numpy().decode()
    mask_path = paths[1].numpy().decode()

    return get_img_765bands(image_path), get_mask_arr(mask_path)
...
def get_landsat_dataset_from_paths(image_paths, masks_paths, batch_size=8, normalization_layer=None, use_data_augmentation=False, shuffle=False, repeat=True):
    ...
    dataset = dataset.map(lambda x:  tf.py_function(open_landsat_image_and_mask, [x], [tf.float32, tf.float32]), num_parallel_calls=tf.data.AUTOTUNE)
