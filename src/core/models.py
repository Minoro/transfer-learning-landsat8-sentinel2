...
"""
Implementation of the models used in this work.
All models was implemented using tensorflow and Keras.

Source for U-net implementation: https://www.depends-on-the-definition.com/unet-keras-segmenting-images/
Source for DeepLab v3+:  https://keras.io/examples/vision/deeplabv3_plus/
Source for SegFormer: https://github.com/ACSEkevin/An-Overview-of-Segformer-and-Details-Description

The models has small changes in the code.
...
from keras.layers import ( 
    Conv2D, BatchNormalization, Activation, Dense, UpSampling2D, Concatenate, 
    AveragePooling2D, Dropout, LayerNormalization, DepthwiseConv2D, Add, Input
)
...
def convolution_block(block_input, num_filters=256, kernel_size=3, dilation_rate=1, use_bias=False, name=None):
    x = Conv2D(
        num_filters,
...
def DilatedSpatialPyramidPooling(dspp_input):
    ...
    x = AveragePooling2D(pool_size=(dims[-3], dims[-2]), name="pool_dspp")(dspp_input)
...
def DeeplabV3Plus(image_size, num_classes, freeze_feature_extractor=False, weights=None):
    ...
    input_b = convolution_block(input_b, num_filters=48, kernel_size=1, name='decoder_conv_low_level_features')
...
def get_unet_with_fuse_model(input_height=256, input_width=256, n_filters = 64, dropout = 0.1, batchnorm = True, n_channels=3, add_contextual_bn=False):
    input_img = Input(shape=(input_height,input_width, n_channels))
    input_unet = input_img

    if add_contextual_bn:
        input_unet = BatchNormalization()(input_img)

    # contracting path
    c1 = conv2d_block(input_unet, n_filters=n_filters*1, kernel_size=3, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2)) (c1)
    p1 = Dropout(dropout)(p1)
    ...
    unet_outputs = Conv2D(1, (1, 1), activation='sigmoid')(c9)

    unet = tf.keras.Model(inputs=[input_img], outputs=[unet_outputs], name='U-net')

    input_fuse = Input(shape=(input_height,input_width, 1), name='input_fuse')
    fuse_i1 = concatenate([input_fuse, c1, c9], axis=3)

    fuse_c1 = conv2d_block(fuse_i1, n_filters=n_filters*1, kernel_size=3, batchnorm=batchnorm)
    fuse_c1 = concatenate([c1, fuse_c1], axis=-1)
    fuse_p1 = MaxPooling2D(pool_size=(2, 2)) (fuse_c1)
    fuse_p1 = Dropout(dropout)(fuse_p1)
    
    fuse_c2 = conv2d_block(fuse_p1, n_filters=n_filters*2, kernel_size=3, batchnorm=batchnorm)

    fuse_u1 = Conv2DTranspose(n_filters*1, (3, 3), strides=(2, 2), padding='same') (fuse_c2)
    fuse_u1 = concatenate([fuse_u1, fuse_c1])
    fuse_u1 = Dropout(dropout)(fuse_u1)
    fuse_c3 = conv2d_block(fuse_u1, n_filters=n_filters*1, kernel_size=3, batchnorm=batchnorm)
    outputs = Conv2D(1, (1, 1), activation='sigmoid') (fuse_c3)

    # outputs = Conv2D(1, (1, 1), activation='sigmoid') (fuse_c1)
    model = tf.keras.Model(inputs=[input_img, input_fuse], outputs=[outputs], name='Fuse')
...
def single_convolutional_block(inputs, n_filters, kernel_size, strides=1, norm=True, activation=None, name=None):
    ...
    if name is not None:
        bn_name = f"{name}_bn"
    x = BatchNormalization(momentum=.99, epsilon=1e-5, name=bn_name)(x) if norm is True else x
...
def decoder_mlp(inputs, embed_dim, name=None):
    ...
    x = Dense(embed_dim, use_bias=True, name=f"{name}_mlp")(x)
...
def seg_former_decoder_block(inputs, embed_dim, up_size=(4, 4), name=None):
    ...
    """
    batches, height, width, channels = inputs.shape
    x = decoder_mlp(inputs, embed_dim, name=name)
...
def seg_former_head(features, embed_dim, num_classes, drop_rate=0.):
    ...
    x = single_convolutional_block(x, n_filters=embed_dim, kernel_size=1, norm=True, activation='relu', name="segformer_head_convblock")
    x = Dropout(rate=drop_rate, name="segformer_head_dropout")(x)

    if num_classes == 1:
        x = Conv2D(num_classes, kernel_size=1, activation='sigmoid', name='segformer_head_output')(x)
    else:
        x = Conv2D(num_classes, kernel_size=1, activation='softmax', name='segformer_head_output')(x)
...
def SegFormer(input_shape,
    ...
    feature1 = seg_former_decoder_block(feature1, decoder_embed_dim, up_size=(4, 4), name="decoder_block_1")
    feature2 = seg_former_decoder_block(feature2, decoder_embed_dim, up_size=(8, 8), name="decoder_block_2")
    feature3 = seg_former_decoder_block(feature3, decoder_embed_dim, up_size=(16, 16), name="decoder_block_3")
    feature4 = seg_former_decoder_block(feature4, decoder_embed_dim, up_size=(32, 32), name="decoder_block_4")
...
def freeze_unet_layers(unet, mode):
    ...
    elif mode == 'freeze_encoder_freeze_bn':

        for layer in unet.layers[:39]:
            layer.trainable = False

        # Congela todas as layers de BN
        for layer in unet.layers:
            if isinstance(layer, tf.keras.layers.BatchNormalization):
                layer.trainable = False
...
def freeze_segformer_layers(model, config):

    if config == 'freeze_all' or config == 'frozen':
        model.trainable = False
        for layer in model.layers:
            layer.trainable = False

    elif config == 'unfrozen' or config == 'unfreeze':
        model.trainable = True
        for layer in model.layers:
            layer.trainable = True

    elif config == 'freeze_encoder' or config == 'frozen_encoder':
        for layer in model.layers:
            if layer.name.startswith('decoder_block') or layer.name.startswith('segformer_head'):
                layer.trainable = False

    else:
        raise ValueError(f'The configuration {config} is not defined for the model')
...
def get_model(name, input_shape, num_classes=1):
    ...
    elif name == 'segformerb5':
        return SegFormerB5(input_shape=input_shape, num_classes=num_classes)
    
    elif name == 'deeplabv3+':
        return DeeplabV3Plus(image_size=input_shape[0], num_classes=num_classes)

    elif name == 'unet':
        return get_unet(input_height=input_shape[0], input_width=input_shape[1])
